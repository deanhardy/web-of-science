...
setwd("C:/Users/dhardy/Dropbox/sesync/manuscripts/under_review_revision/sharing_meanings_approach/R")
...
## import data
vuln <- read.delim(file.path(datadir, "data/vuln.txt"), header = TRUE, sep = "\t") %>%
  head(., -2) %>%
  mutate(count = records, year = as.character(Publication.Years)) %>%
  mutate(year = year(as.Date(year, "%Y"))) %>%
  select("year", "count")

vulnhd <- read.delim(file.path(datadir, "data/vulnhd.txt"), header = TRUE, sep = "\t") %>%
  head(., -2) %>%
  mutate(count = records, year = as.character(Publication.Years)) %>%
  mutate(year = year(as.Date(year, "%Y"))) %>%
  select("year", "count")
...
fig <- ggplot(filter(df, year < 2018 & year > 1964), aes(year, records/1000, group = identity)) + 
